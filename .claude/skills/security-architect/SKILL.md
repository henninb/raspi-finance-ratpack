...
1. **Threat model the code** — before reviewing any code, produce:
   ...
   - The attack surface: every entry point (REST endpoints, file uploads, background jobs)
...
5. **Language-specific patterns to check** (apply whichever is relevant to the language in scope):
   ...
   - **Groovy (Ratpack / jOOQ)**: `"command".execute()` or backtick shell execution with user-controlled input, `Eval.me()` or `GroovyShell.evaluate()` with user input, `ObjectInputStream` deserialization via Java interop, dynamic method dispatch via `invokeMethod` with user-controlled method names, GString SQL interpolation bypassing jOOQ parameterization (always use jOOQ DSL bind variables), missing CORS origin allowlist in `CorsHandler` (overly broad `*`), hardcoded credentials in `db_config.json` or application source, missing authentication middleware on Ratpack handler chains, `YAML.load` (use `safe_load`) in Groovy config parsing
...
10. **Business logic security (financial applications)**:
    ...
    - Flag use of `Float` or `Double` for monetary values — floating point arithmetic produces rounding errors; require `BigDecimal` with explicit scale and `RoundingMode`
